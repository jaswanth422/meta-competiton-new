...
* Minimum 3 tasks with agent graders (easy → medium → hard, scores strictly in (0, 1))
* Meaningful reward function with partial progress signals
* Baseline inference script with reproducible scores
* Deploy to Hugging Face Spaces + working Dockerfile
* README with environment description, action/observation spaces, setup instructions

# Updated Scoring Rule (Fixed)

## Task & Grader Requirements (Updated)
...
## Grader Implementation Rule (Updated)

Graders must:

- Return continuous scores in (0, 1)
- Avoid binary outputs like 0.0 or 1.0
- Be deterministic and reproducible

### Example (Correct)

```python
def grade_task(success: bool) -> float:
  if success:
    return 0.999
  else:
    return 0.001
...
Each task defines a concrete objective an agent must accomplish, with a programmatic grader that scores performance strictly between 0 and 1. Tasks should range: easy → medium → hard. Graders must have clear, deterministic success/failure criteria.
...
•  Graders produce scores strictly in (0, 1)?
...
Enumerate tasks, run each grader, verify scores are strictly in (0, 1)
...
- The script must emit exactly three line types to stdout, in this order:
  ...
  [END] success=true steps=3 score=0.999 rewards=0.00,0.00,1.00
  """
...
SCORE_EPSILON = 1e-3  # keeps final score strictly inside (0, 1)
...
async def main() -> None:
    ...
    score = sum(rewards) / MAX_TOTAL_REWARD if MAX_TOTAL_REWARD > 0 else SCORE_EPSILON
      score = min(max(score, SCORE_EPSILON), 1.0 - SCORE_EPSILON)  # clamp to (0, 1)
